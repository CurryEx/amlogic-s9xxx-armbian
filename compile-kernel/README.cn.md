...
💡注意: 如果你 `fork` 仓库并进行了修改，使用时须将 Actions 的 `用户名` 改成你自己的仓库，并根据说明中的第 2-3 条 [添加 TOKEN或使用Artifacts](../build-armbian/documents/README.cn.md#2-设置隐私变量-github_token) 对编译好的文件上传位置进行设置，例如：
...
| build_files_upload_option | false            | 如果您是个人编译，勾选此选项即可输出到Action的Artifacts，无需设置token。注意Artifacts保留14天                                                                                    |
