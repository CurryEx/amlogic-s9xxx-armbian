...
After the firmware is compiled, we need to upload the firmware. For personal use, you can check the latest option while start Workflows without any token setting below. However, in this situation, you can not use these tiles on other Actions Workflows. After uploading, reference [7. Download the firmware](#7-download-the-firmware) to download.

![option](https://user-images.githubusercontent.com/46918772/218275632-5ab3db20-f9a9-43c7-a298-35240c5d08de.png)

For upload compile files to GitHub Releases, you should set tokens first. Set the GitHub privacy variable `GitHub_TOKEN`.  to `GitHub Releases`. We set this variable according to the official requirements of GitHub. The method is as follows:
...
If you checked upload to `Artifacts`, you can find files on Actions > *The Workflow You Run* > Summary > Artifacts

![image](https://user-images.githubusercontent.com/46918772/218276112-0f5587fe-4b03-4b61-aef0-2d175d90ba0d.png)

Otherwise, enter from the GitHub `Releases` section in the bottom right corner of the `Repository homepage`, and select the firmware corresponding to the model of your `Amlogic s9xxx TV Boxes`. The icons are as follows:
